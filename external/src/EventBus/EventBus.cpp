...
std::size_t EventBus::processLimit(const std::size_t limit) {
...

    {
        ...
        if (_eventStreams.size() != _eventToStream.size()) {
            auto removeFrom =
                std::remove_if(_eventStreams.begin(), _eventStreams.end(), [this](const auto& eventStream) {
...
eventbus::stream::EventStream* EventBus::findStream(const eventbus::internal::event_id_t eventID) const {
...
eventbus::stream::EventStream* EventBus::findStreamUnsafe(const eventbus::internal::event_id_t eventID) const {
    auto lookup = _eventToStream.find(eventID);
    return lookup != _eventToStream.end() ? lookup->second : nullptr;
}

eventbus::stream::EventStream* EventBus::obtainStream(const eventbus::internal::event_id_t eventID,
...
eventbus::stream::EventStream* EventBus::streamForEvent(eventbus::internal::event_id_t eventID) const {
    std::lock_guard writeGuard{_mutexStreams};
    auto*           found = findStreamUnsafe(eventID);
    if (found != nullptr) {
        return found;
...
eventbus::stream::EventStream* EventBus::listen(const std::uint32_t, const eventbus::internal::event_id_t eventID,
                                                eventbus::CreateStreamCallback createStreamCallback) {
    auto* eventStream = obtainStream(eventID, createStreamCallback);
    return eventStream;
}

void EventBus::unlisten(const std::uint32_t listenerID, const eventbus::internal::event_id_t eventID) {
