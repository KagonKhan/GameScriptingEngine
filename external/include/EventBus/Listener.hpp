...
#include "dexode/listener_traits.hpp"
...
template <class Bus> class Listener {
public:
    ...
    template <class Event, typename _ = void> constexpr void listen(std::function<void(const Event&)>&& callback) {
    ...
    template <class Event> void listenToCallback(std::function<void(const Event&)>&& callback) {
        static_assert(internal::validateEvent<Event>(), "Invalid event");
        if (_bus == nullptr) {
    ...
    template <class Event> void listenToCallback(const std::function<void(const Event&)>& callback) {
        static_assert(internal::validateEvent<Event>(), "Invalid event");
        if (_bus == nullptr) {
            throw std::runtime_error{"bus is null"};
        }
        internal::ListenerAttorney<Bus>::template listen<Event>(*_bus, _id,
    ...
    template <typename Event> void unlisten() {
        static_assert(internal::validateEvent<Event>(), "Invalid event");
        if (_bus == nullptr) {
            throw std::runtime_error{"bus is null"};
        }
    ...
    template <typename Event> [[nodiscard]] bool isListening() const {
        static_assert(internal::validateEvent<Event>(), "Invalid event");
        if (_bus == nullptr) {
            throw std::runtime_error{"bus is null"};
        }
        return internal::ListenerAttorney<Bus>::isListening(*_bus, _id, internal::event_id<Event>());
    }
