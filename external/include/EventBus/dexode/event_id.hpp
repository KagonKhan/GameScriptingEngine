...
template <class Event> constexpr bool validateEvent() {
    static_assert(std::is_const_v<Event> == false, "Struct must be without const");
    static_assert(std::is_volatile_v<Event> == false, "Struct must be without volatile");
    static_assert(std::is_reference_v<Event> == false, "Struct must be without reference");
    static_assert(std::is_pointer_v<Event> == false, "Struct must be without pointer");
    return true;
}
