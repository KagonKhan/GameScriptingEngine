...
template <typename EventBus_t> class ListenerAttorney {
    template <typename> friend class dexode::eventbus::Listener;

private:
    static constexpr std::uint32_t newListenerID(EventBus_t& bus) { return bus.newListenerID(); }

    template <class Event>
    static constexpr void listen(EventBus_t& bus, const std::uint32_t listenerID,
    ...
    static constexpr void unlisten(EventBus_t& bus, const std::uint32_t listenerID, const event_id_t eventID) {
        bus.unlisten(listenerID, eventID);
    }

    static constexpr bool isListening(EventBus_t& bus, const std::uint32_t listenerID, const event_id_t eventID) {
