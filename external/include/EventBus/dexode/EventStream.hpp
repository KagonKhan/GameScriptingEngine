...
public:
    ...
    bool   addListener(std::uint32_t listenerID, std::any callback) override { throw std::runtime_error{"Noop"}; }
    bool   removeListener(std::uint32_t listenerID) override { throw std::runtime_error{"Noop"}; }
    [[nodiscard]] bool hasListener(std::uint32_t listenerID) const override { throw std::runtime_error{"Noop"}; }
};
