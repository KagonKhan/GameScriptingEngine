...
#include "dexode/ListenerAttorney.hpp"
#include "dexode/ProtectedEventStream.hpp"
...
template <typename Event> using DefaultEventStream = eventbus::stream::ProtectedEventStream<Event>;
...
template <typename Event> bool postpone(Bus& bus, std::any event);

template <typename Event> std::unique_ptr<eventbus::stream::EventStream> createDefaultEventStream() {
...
public:
    ...
    template <typename Event> static PostponeHelper create(std::any&& event) {
        return PostponeHelper{internal::event_id<Event>(), std::forward<std::any>(event), postpone<Event>,
...
class Bus {
    template <typename> friend class dexode::eventbus::internal::ListenerAttorney;
...
public:
    ...
    template <typename Event> bool postpone(Event event) {
...
protected:
    ...
    virtual eventbus::stream::EventStream* listen(std::uint32_t listenerID, internal::event_id_t eventID,
    ...
    virtual eventbus::stream::EventStream* streamForEvent(eventbus::internal::event_id_t eventID) const = 0;
...
private:
    ...
    template <class Event> void listen(const std::uint32_t listenerID, std::function<void(const Event&)>&& callback) {
        static_assert(internal::validateEvent<Event>(), "Invalid event");
